...
export default function AcademyLanding() {
  ...
  return (
    <div className="min-h-screen bg-white text-slate-900 selection:bg-[#ff5500]/30">
      ...
      {/* ── 3.5 FOUNDATION PREP SECTION ── */}
      <section className="relative py-20 bg-gradient-to-br from-white to-[#f4f7fc] overflow-hidden">
        {/* Background blobs */}
        <div className="absolute top-1/4 left-[-10%] w-[35%] h-[35%] rounded-full bg-[#5a4bda]/5 blur-3xl pointer-events-none" />
        <div className="absolute bottom-1/4 right-[-10%] w-[35%] h-[35%] rounded-full bg-[#ff5500]/5 blur-3xl pointer-events-none" />
        ...
        <div className="max-w-7xl mx-auto px-6 relative z-10">
          <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 items-center">
            ...
            <div className="lg:col-span-7 space-y-8 text-center lg:text-left">
              <h2 className="text-4xl md:text-5xl lg:text-6xl font-black text-slate-900 leading-[1.15] tracking-tight">
                ...
                <span className="text-[#5a4bda]">Curious Minds!</span>
              </h2>

              <p className="text-lg md:text-xl text-slate-600 font-semibold max-w-xl mx-auto lg:mx-0">
                Let your child start learning how to excel in School Curriculum, Competitive Science &amp; Logical Mathematics!
              </p>

              {/* Class target pill with custom vector underline */}
              <div className="inline-block relative py-1 px-2">
                <span className="text-xl md:text-2xl font-black text-[#ff5500] tracking-tight">
                  For classes 1st to 10th
                </span>
                {/* Curved underline SVG */}
                <svg className="absolute bottom-[-10px] left-0 w-full h-3 text-[#ff5500]" viewBox="0 0 300 12" fill="none" xmlns="http://www.w3.org/2000/svg" preserveAspectRatio="none">
                  <path d="M5 9C100 2 200 2 295 9" stroke="currentColor" strokeWidth="4" strokeLinecap="round" />
                </svg>
              </div>

              <div className="pt-6">
                <Link href="#courses">
                  <Button size="lg" className="px-10 h-14 text-base font-extrabold bg-[#ff5500] hover:bg-[#e04b00] hover:scale-105 transition-all text-white rounded-xl shadow-lg shadow-[#ff5500]/25 border-0">
            ...
            {/* Right Side: High-fidelity study illustration */}
            <div className="lg:col-span-5 relative flex justify-center items-center h-[420px]">
              
              {/* Spinning/bobbing educational doodles behind the profile */}
              <div className="absolute w-[340px] h-[340px] rounded-full border border-dashed border-slate-200 animate-[spin_50s_linear_infinite]" />

              {/* Main Avatar container with circular photo */}
              <motion.div 
                animate={{ y: [0, -10, 0] }}
                transition={{ duration: 6, repeat: Infinity, ease: "easeInOut" }}
                className="relative z-10 w-[280px] h-[280px] rounded-[60px] overflow-hidden border-8 border-white shadow-2xl shadow-slate-900/10"
              >
                <Image 
              ...
              {/* Floating SVGs & Doodles */}
              {/* Dotted path 1 */}
              <svg className="absolute w-[180px] h-[100px] text-[#ff5500]/40 pointer-events-none z-20 left-4 top-12" viewBox="0 0 100 50" fill="none" stroke="currentColor" strokeWidth="1.5" strokeDasharray="3 3">
                <path d="M5,45 Q30,5 60,35 T95,10" />
              </svg>
              {/* Dotted path 2 */}
              <svg className="absolute w-[150px] h-[80px] text-[#5a4bda]/40 pointer-events-none z-20 right-6 bottom-16" viewBox="0 0 100 50" fill="none" stroke="currentColor" strokeWidth="1.5" strokeDasharray="3 3">
                <path d="M5,10 Q40,40 70,10 T95,30" />
              </svg>

              {/* Doodled Chemistry Flask */}
              <motion.div 
                animate={{ rotate: [-8, 8, -8] }}
                transition={{ duration: 4, repeat: Infinity, ease: "easeInOut" }}
                className="absolute bottom-20 right-2 w-12 h-12 bg-white rounded-2xl shadow-lg border border-slate-100 flex items-center justify-center text-xl z-20"
              >
                🧪
              </motion.div>
              ...
              <motion.div 
                ...
                className="absolute top-20 left-2 w-12 h-12 bg-white rounded-2xl shadow-lg border border-slate-100 flex items-center justify-center text-xl z-20"
              >
                ⚛️
              </motion.div>
              ...
              <motion.div 
                ...
                className="absolute top-6 right-16 w-12 h-12 bg-white rounded-2xl shadow-lg border border-slate-100 flex items-center justify-center text-xl z-20"
              >
                📖
              </motion.div>

              {/* Geometry block Doodle */}
              <motion.div 
                animate={{ y: [4, -4, 4] }}
                transition={{ duration: 5, repeat: Infinity, ease: "easeInOut" }}
                className="absolute bottom-10 left-16 w-12 h-12 bg-white rounded-2xl shadow-lg border border-slate-100 flex items-center justify-center text-xl z-20"
              >
                📐
              </motion.div>
          ...
          {/* Bottom features bar */}
          <div className="mt-16 bg-white rounded-2xl border border-slate-100 shadow-[0_15px_50px_-15px_rgba(0,0,0,0.05)] p-6 md:p-8">
            <div className="grid grid-cols-2 md:grid-cols-4 gap-6 divide-y md:divide-y-0 md:divide-x divide-slate-100">
              
              {/* Feature 1 */}
              <div className="text-center md:px-4 flex flex-col items-center gap-3 pt-4 md:pt-0">
                <div className="w-12 h-12 rounded-2xl bg-rose-50 flex items-center justify-center text-xl text-rose-500 font-bold shadow-sm shadow-rose-500/10">
                  🔴
                </div>
                <div>
                  <h4 className="font-extrabold text-slate-900 text-sm md:text-base">Live Interactive</h4>
                  <p className="text-xs text-slate-500 font-medium mt-0.5">Two-way virtual classes</p>
                </div>
              </div>

              {/* Feature 2 */}
              <div className="text-center md:px-4 flex flex-col items-center gap-3 pt-4 md:pt-0">
                <div className="w-12 h-12 rounded-2xl bg-blue-50 flex items-center justify-center text-xl text-blue-500 font-bold shadow-sm shadow-blue-500/10">
                  ✍️
                </div>
                <div>
                  <h4 className="font-extrabold text-slate-900 text-sm md:text-base">24 x 7 Support</h4>
                  <p className="text-xs text-slate-500 font-medium mt-0.5">Continuous mentor guide</p>
                </div>
              </div>

              {/* Feature 3 */}
              <div className="text-center md:px-4 flex flex-col items-center gap-3 pt-4 md:pt-0">
                <div className="w-12 h-12 rounded-2xl bg-purple-50 flex items-center justify-center text-xl text-purple-500 font-bold shadow-sm shadow-purple-500/10">
                  🧠
                </div>
                <div>
                  <h4 className="font-extrabold text-slate-900 text-sm md:text-base">Daily Tracking</h4>
                  <p className="text-xs text-slate-500 font-medium mt-0.5">Personal progress dashboard</p>
                </div>
              </div>

              {/* Feature 4 */}
              <div className="text-center md:px-4 flex flex-col items-center gap-3 pt-4 md:pt-0">
                <div className="w-12 h-12 rounded-2xl bg-amber-50 flex items-center justify-center text-xl text-amber-500 font-bold shadow-sm shadow-amber-500/10">
                  🏅
                </div>
                <div>
                  <h4 className="font-extrabold text-slate-900 text-sm md:text-base">Practice Led</h4>
                  <p className="text-xs text-slate-500 font-medium mt-0.5">Activity-based learning</p>
                </div>
              </div>
